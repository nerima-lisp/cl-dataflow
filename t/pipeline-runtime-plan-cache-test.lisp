...
before MUTATION ran."
  (let ((plan (gensym "PLAN-")))
    `(let ((,plan (cl-dataflow-kit::%pipeline-execution-plan ,pipeline)))
        ,@mutation
        (cl-dataflow-kit::%ensure-pipeline-execution-plan ,pipeline)
        (is (not (eq ,plan (cl-dataflow-kit::%pipeline-execution-plan ,pipeline)))))))
...
(deftest
  ...
  (with-linear-test-pipeline
    ...
    (let ((plan (cl-dataflow-kit::%pipeline-execution-plan pipeline)))
      ...
      (is (eq plan (cl-dataflow-kit::%pipeline-execution-plan pipeline)))
      (run-pipeline pipeline)
      (is (eq plan (cl-dataflow-kit::%pipeline-execution-plan pipeline))))))
...
(deftest
  ...
  (let* ((graph (make-graph))
    ...
    (let* ((pipeline (make-pipeline :graph graph :stages (list second first)))
            (plan (cl-dataflow-kit::%pipeline-execution-plan pipeline))
            (stages (cl-dataflow-kit::%pipeline-execution-plan-stages plan))
            (sinks (cl-dataflow-kit::%pipeline-execution-plan-sinks plan)))
...
(deftest
  ...
  (let* ((graph (make-graph))
    ...
    (let* ((pipeline (make-pipeline :graph graph :stages (list a c b)))
            ...
            (edge (first (cl-dataflow-kit::%graph-edges-list live-graph)))
            (old-plan (cl-dataflow-kit::%pipeline-execution-plan pipeline)))
      ...
      (is (not (eq old-plan (cl-dataflow-kit::%pipeline-execution-plan pipeline)))))))
...
(deftest
  ...
  (with-linear-test-pipeline
    (graph pipeline source sink)
    (setf (node-name (first (cl-dataflow-kit::%pipeline-stages-list pipeline))) "renamed")
...
(deftest
  ...
  (with-linear-test-pipeline
        (graph pipeline source sink)
        (let ((original-plan (cl-dataflow-kit::%pipeline-execution-plan pipeline)))
          (setf (pipeline-stages pipeline) (pipeline-stages pipeline))
          (is (null (cl-dataflow-kit::%pipeline-execution-plan pipeline)))
          (cl-dataflow-kit::%ensure-pipeline-execution-plan pipeline)
          (is (not (eq original-plan (cl-dataflow-kit::%pipeline-execution-plan pipeline))))
          (let ((copy (copy-pipeline pipeline)))
            (is
              (not
                (eq
                  (cl-dataflow-kit::%pipeline-execution-plan pipeline)
                  (cl-dataflow-kit::%pipeline-execution-plan copy))))
            (is
              (not
                (eq
                  (cl-dataflow-kit::%pipeline-execution-plan-graph
                    (cl-dataflow-kit::%pipeline-execution-plan pipeline))
                  (cl-dataflow-kit::%pipeline-execution-plan-graph
                    (cl-dataflow-kit::%pipeline-execution-plan copy)))))))))
...
(deftest
  ...
  (with-linear-test-pipeline
    ...
    (let ((original-plan (cl-dataflow-kit::%pipeline-execution-plan pipeline)))
      ...
      (is (not (eq original-plan (cl-dataflow-kit::%pipeline-execution-plan pipeline))))
      (is (null (cl-dataflow-kit::%pipeline-execution-plan-stages
                  (cl-dataflow-kit::%pipeline-execution-plan pipeline)))))))
...
(deftest
  ...
  (let* ((old-value 1)
    ...
    (let* ((pipeline
          (make-pipeline :graph graph :stages (list old-source new-source sink)))
            (plan (cl-dataflow-kit::%pipeline-execution-plan pipeline))
            (binding-plans (cl-dataflow-kit::%pipeline-execution-plan-input-binding-plans plan))
  ...
  (let* ((source-value (list (cons "key" 42)))
    ...
    (let* ((pipeline (make-pipeline :graph graph :stages (list source sink)))
            ...
            (context (make-context)))
      (run-pipeline pipeline :input :pipeline-input :context context)
      (is (eq seen-input (cl-dataflow-kit::%read-value context source (edge-from-port live-edge))))
      (is (eq seen-input (getf (first (cl-dataflow-kit::%context-trace-list context)) :input)))
...
(deftest
  ...
  (let* ((graph (make-graph))
    ...
    (let* ((pipeline (make-pipeline :graph graph :stages (list source sink)))
      ...
      (let ((old-plan (cl-dataflow-kit::%ensure-pipeline-execution-plan pipeline)))
        (is (= (run-pipeline pipeline) 42))
        (is (= seen-input 42))
        (setf seen-input :not-run)
        (setf (char port 0) #\x)
